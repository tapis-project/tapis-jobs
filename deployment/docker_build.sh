...
echo "Creating images for local testing using tags: $TAG_LOCAL1 and $TAG_LOCAL3"
...
if [ "x$2" = "x-push" ]; then
  if [ "$ENV" = "prod" ]; then
    echo "Creating third image tags for prod env: $TAG_LATEST1 and $TAG_LATEST3"
  ...
  if [ "x$TAPIS_DEPLOY_MANUAL" = "xtrue" ]; then
    echo "Creating ENV image tags: $TAG_ENV1 and $TAG_ENV3"
