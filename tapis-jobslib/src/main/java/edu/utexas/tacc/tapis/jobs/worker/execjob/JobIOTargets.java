 ...
 *
 *  These directories are often local file systems on the execution system, but they can also be remote directories
 *  mounted on the execution system. In addition, these remote directories can be designated as residing on Data
 *  Transfer Nodes (DTNs), which are specially defined Tapis systems that are used because of their high IO capacity.
 *  When an execution system designates that a DTN should be used for IO, Jobs will direct the files service to
 *  read and/or write data directly to the DTN host rather than to the execution host to improve IO performance.
 *  
 *  A DTN is assigned a mount-point on an execution system. If any of the above listed three directory paths start with
 *  the mount-point directory, then IO will target the DTN directly rather than the execution system. To do this, Jobs
 *  substitutes the DTN system as the target system and replaces the mount-point prefix in the directory path with the
 *  source path on the DTN used for that mount. When data are read from or written to the DTN directory, they will be
 *  accessible on the execution system in directories rooted at the mount-point.
 *  
 *  The role of this class is two fold.
 *  One purpose is to conveniently collect all directories that must be created during Job execution.
 *  The other purpose is to figure out in which cases the DTN should be targeted and in which class the execution
 *  should be targeted. The mentioned adjustments to systems and directories are performed once upon initialization
...
public final class JobIOTargets 
{
    ...
    private final TapisSystem         _archiveSystem;
    ...
    private final JobIOTarget         _archiveTarget = new JobIOTarget();
    ...
    /* constructor:                                                           */
    /* ---------------------------------------------------------------------- */
    public JobIOTargets(Job job, TapisSystem execSystem, TapisSystem archiveSystem, TapisSystem dtnSystem)
    ...
    public JobIOTarget getArchiveTarget() {return _archiveTarget;}
    ...
    private void initSystemsAndDirs()
    {
        ...
        // We always write directly to the archive directory.
        _archiveTarget.systemId       = _job.getArchiveSystemId();
        _archiveTarget.host           = _archiveSystem.getHost();
        _archiveTarget.dir            = _job.getArchiveSystemDir();
    }
